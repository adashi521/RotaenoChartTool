...
void Rotate::setValue(const NoteAttr attr, const double value) {
    switch (attr) {
    case NoteAttr::time:
        time = value;
        break;
...
void Rotate::setVector(const std::deque<double>& deq) {
    size_t pos = 0;
...
std::shared_ptr<Note> Rotate::clone() const {
    return std::make_shared<Rotate>(*this);
}

double Rotate::getValue(const NoteAttr attr) const {
    switch (attr) {
    case NoteAttr::time:
        return time;
        break;
